...
- Video and the Plex home library now live in the footage.center repo (`research/2026-09-14-plex-home-library.md`). From this repo's side the only question is disk: the Videos folder (47.5 GiB) leaves C: when it moves to the SSD.
- Done 2026-09-14: stale git worktrees. After a live re-check of each one, 32 were removed through their parent repo (every commit on a remote, and no dirty files beyond three the agent sync tool rewrites) and 4 orphan directories whose parent repos had moved were deleted. Worktrees holding commits on no remote or real uncommitted edits were kept, including summon.company-sum-196, summon-sum-170-d1, summon.guide, sprite.email, youchop.app, and anchormarianas.com.
